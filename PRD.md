...
> **当前实现口径（2026-06-14）**：上表为早期产品草案，现行服务统一使用 `/api/v1/workflows`。已实现列表、创建、详情、更新、删除、部署、手动触发、工作流执行记录、全局执行记录、执行详情、取消执行共 11 个核心接口；另提供 `POST /api/v1/workflows/events/{eventType}` 触发事件型工作流。定时型工作流通过部署后的 `triggerConfig.cron` 自动调度。编辑器当前支持 `START`、`AGENT`、`CONDITION`、`PARALLEL`、`END` 节点，并在保存和部署前校验起止节点、连通性、环路及节点参数。
...
> **当前实现口径（2026-06-14）**：测评与数据集接口已统一迁移至 `/api/v1`。数据集使用 `/api/v1/datasets`，支持导入、生成、预览、删除和批量删除；测评使用 `/api/v1/evaluations`，支持单任务、批量任务、进度、详情、结果、取消、重试及 JSON/CSV 文件导出。批量创建使用 `POST /api/v1/evaluations/batch`，批次查询使用 `GET /api/v1/evaluations/batch/{batchCode}`，聚合结果使用 `GET /api/v1/evaluations/{id}/results`。`sampleCount=0` 表示使用全部样本，允许范围为 1-10000；`timeout` 的单位为毫秒。当前不承诺 PDF/Excel 测评报告导出。
